...
// The JExtractNotInBrowserError class
class JExtractNotInBrowserError extends JExtractError {
    constructor() {
        super("This is not possible because there is no browser environment.");
        this.name = "JExtractNotInBrowserError";
    }
}
            ...
            if (Is.undefined(substruct)) {
                ...
                if (Is.string(extractedData) && filter[0]) {
                    ...
                    if (Is.function(filter[0])) {
                        method = filter[0];
                        args = [extractedData.get(), index].concat(filter[1]);
